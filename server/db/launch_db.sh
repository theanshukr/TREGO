#!/usr/bin/env bash
# Launch Postgres + pgvector for trego. Persists data in a named volume so
...
NAME="${NAME:-trego-db}"
PORT="${PORT:-5432}"
VOLUME="${VOLUME:-trego-db-data}"
PASSWORD="${POSTGRES_PASSWORD:-trego}"
USER="${POSTGRES_USER:-trego}"
DB="${POSTGRES_DB:-trego}"
