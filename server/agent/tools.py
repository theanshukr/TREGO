...
_ALLOWED_TOOLS: set[str] = {
    ...
    "get_specific_instructions", "run_dev_cmd",
    # system controls
...
#
# Why not the CWD? `./trego-audit.log` resolves to wherever the launcher
# was started, which is also a directory the user (and any process
# running as the user) can write to. An attacker process could
# overwrite the log to hide what trego did. Default to
# %PROGRAMDATA%\trego\audit.log on Windows — that directory is created
# by us and we lock down its ACL on first use. Override with
# TREGO_AUDIT_LOG=<absolute path>.
def _default_audit_path() -> Path:
    override = os.getenv("TREGO_AUDIT_LOG", "").strip()
    if override:
        return Path(override).resolve()
    if os.name == "nt":
        base = Path(os.getenv("PROGRAMDATA", r"C:\ProgramData"))
        return (base / "trego" / "audit.log").resolve()
    return (Path.home() / ".trego" / "audit.log").resolve()


_AUDIT_PATH = _default_audit_path()
_AUDIT_ROTATE_BYTES = int(os.getenv("TREGO_AUDIT_ROTATE_BYTES", str(5 * 1024 * 1024)))
...
def _validate_args(call: ToolCall) -> dict[str, Any] | None:
    ...
    elif name == "run_dev_cmd":
        command = str(args.get("command", "")).strip()
        if not command:
            return {"error": "run_dev_cmd requires non-empty 'command' argument"}
        from shared.command_policy import evaluate_command
        eval_res = evaluate_command(command)
        if not eval_res.is_allowed:
            return {"error": f"run_dev_cmd rejected by command policy: {eval_res.reason}"}
...
TOOL_SCHEMAS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            "name": "run_dev_cmd",
            "description": "Execute an allowed developer CLI or diagnostic command (e.g. g++ --version, flutter doctor -v, python -m venv, git clone). Policy-checked and risk-gated.",
            "parameters": {
                "type": "object",
                "properties": {
...
def dispatch(call: ToolCall, computer: Any, mode: str = "control") -> dict[str, Any]:
    ...
    if name not in _ALLOWED_TOOLS:
        result = {"error": f"tool '{name}' is not on the trego allow-list"}
