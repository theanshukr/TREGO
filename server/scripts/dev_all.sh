#!/usr/bin/env bash
# Backend-side one-shot launcher for trego. Idempotent.
#
...
if ! docker image inspect trego-backend:latest >/dev/null 2>&1; then
    log "building trego-backend image (one-time)"
...
COUNT=$("${COMPOSE[@]}" exec -T postgres psql -U "${POSTGRES_USER:-trego}" -d "${POSTGRES_DB:-trego}" -tA \
