...
from shared.command_policy import evaluate_command, CommandRisk
...
def _run_ps_internal(
    ...
    *,
    ...
    Inside `script`, reference user-controlled values as `$env:TREGO_X`
...
# Window-title substrings that mean "this is the trego chat UI" and should
# be hidden from screenshots so the model doesn't try to click its own logs.
_TREGO_TITLE_MARKERS = ("trego", "trego")


def _hide_trego_windows() -> int:
    """Minimize every visible top-level window whose title looks like the
    trego chat. Best-effort: any failure is swallowed so a screenshot still
    ...
    for w in wins:
        ...
        if not any(m in title for m in _TREGO_TITLE_MARKERS):
            continue
        try:
...
def screenshot() -> dict[str, Any]:
    ...
    Set TREGO_HIDE_OWN_WINDOWS=1 to minimize trego windows before
    ...
    hide = os.getenv("TREGO_HIDE_OWN_WINDOWS", "").lower() in ("1", "true", "yes")
    hidden = _hide_trego_windows() if hide else 0
    if hidden:
        time.sleep(0.25)
...
"""
    proc = _run_ps_internal(script, env={"TREGO_TITLE": window_title})
...
def write_clipboard(text: str) -> dict[str, Any]:
    ...
    script = "Set-Clipboard -Value $env:TREGO_CLIP"
    _run_ps_internal(script, env={"TREGO_CLIP": text})
...
def search_files(query: str, directory: str = ".") -> dict[str, Any]:
    ...
    script = (
        "Get-ChildItem -Path $env:TREGO_DIR -Recurse -File "
        "-ErrorAction SilentlyContinue | "
        "Select-String -Pattern $env:TREGO_Q -SimpleMatch | "
        "Select-Object -First 50 Path, LineNumber, Line | "
        "ConvertTo-Json"
    )
    proc = _run_ps_internal(
        script,
        env={"TREGO_DIR": str(resolved_dir), "TREGO_Q": query},
        timeout=40,
    )
    import json
    try:
...
def read_file_preview(file_path: str) -> dict[str, Any]:
    ...
    script = "Get-Content -Path $env:TREGO_PATH -TotalCount 500 -ErrorAction SilentlyContinue | Out-String"
    proc = _run_ps_internal(script, env={"TREGO_PATH": str(resolved)})
...
def open_assistant() -> dict[str, Any]:
    """Pop the floating trego widget and minimize ALL windows.
    ...
    the trego tab) gets the user's desktop into a clean state so the
    ...
    with _assistant_lock:
        ...
        if not alive:
            here = pathlib.Path(__file__).resolve().parent.parent
            script = here / "scripts" / "trego_assistant.py"
            if not script.exists():
                return {"launched": False, "minimized": 0, "error": f"missing {script}"}
            log_path = pathlib.Path(tempfile.gettempdir()) / "trego_assistant_stderr.log"
...
def _minimize_trego_browser_tabs() -> int:
    """Minimize every top-level window whose title contains 'trego'
...
def run_dev_cmd(
    ...
    """Execute a developer/diagnostic CLI command subject to TREGO Command Policy.
    
    Validates against risk classification and allowlist before execution.
    Captures stdout, stderr, exit code and duration.
    """
    eval_res = evaluate_command(command)
    if not eval_res.is_allowed:
        return {
            "ok": False,
            "error": f"Security policy rejected command: {eval_res.reason}",
            "risk": eval_res.risk.value,
        }

    # Verify working directory if provided
    exec_cwd = None
    if cwd:
        try:
            exec_cwd = validate_file_path(cwd)
        except Exception as e:
            return {"ok": False, "error": f"Invalid cwd: {e}"}
