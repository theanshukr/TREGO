...
# Where the trego backend lives. Set BACKEND_WS_URL=ws://<backend-host>:7860
...
CONFIG_FILE    = os.path.join(_REPO_ROOT, "trego_assistant_config.json")
ROBOT_SIZE     = 72
...
class TREGOAssistant(QWidget):
    def __init__(self):
        ...
        # Persistent WebSocket to the trego backend. Lives in a daemon thread.
    ...
    def _build_ui(self):
        ...
        # Mode toggle: "control" (default, trego acts) vs "guide" (trego only
        ...
        self.mode_btn.setToolTip("Click: Control My Computer (trego acts)\nClick again: Show Me What to Click")
    ...
    def _toggle_mode(self):
        if self.mode == "control":
            ...
            self.mode_btn.setToolTip("Mode: Control My Computer (trego acts)\nClick to switch to Show Me What to Click")
    ...
    def send_prompt(self):
        ...
        with self._ws_lock:
            if self._ws is None:
                self._on_err("Not connected to trego backend yet — try again in a sec.")
                return
            try:
                self._ws.send(msg)
    ...
    def _on_permission_request(self, tool: str):
        ...
        try:
            ...
            box.setWindowTitle("trego — permission required")
            box.setText("🖱️⌨️  Allow trego to use the keyboard and mouse?")
    ...
    def _on_thought(self, text: str, stream: bool):
        ...
        if stream:
            ...
            self._streaming_label = self._add_bubble("TREGO", "💭 Thinking...")
        else:
            self._streaming_label = None
            self._add_bubble("TREGO", f"💭 {text}")
    ...
    def _on_tool_call(self, name: str, args: dict):
        ...
        self._add_bubble("TREGO", f"▶ {name}({arg_str})")
    ...
    def _on_result(self, success: bool, summary: str):
        ...
        role = "TREGO" if success else "System"
...
def _crash_to_messagebox(exc_type, exc, tb):
    ...
    via pythonw.exe. Logs to %TEMP%\\trego_assistant.log."""
    import traceback, tempfile
    msg = "".join(traceback.format_exception(exc_type, exc, tb))
    log = os.path.join(tempfile.gettempdir(), "trego_assistant.log")
    try:
    ...
    try:
        ...
        ctypes.windll.user32.MessageBoxW(
            0, msg[:1500] + f"\n\nFull log: {log}",
            "trego assistant — error", 0x10,  # MB_ICONERROR
        )
    except Exception:
