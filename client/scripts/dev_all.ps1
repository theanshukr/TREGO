# Windows-side one-shot launcher for trego. Idempotent.
#
...
if ($NoAssistant) { $env:TREGO_AUTOLAUNCH_ASSISTANT = "0" }
else              { $env:TREGO_AUTOLAUNCH_ASSISTANT = "1" }

Ok "starting trego executor (Ctrl-C to stop)"
